function applyQuickFill(app, opts)
    ...
    %   DYNAMOApp('EDFPath','/x','StagingPath','/x', ...
    ...
    if ~isempty(opts.EDFPath)
        ...
        if isfolder(folder)
            ...
            if ~isempty(files)
                ...
                warning('DYNAMOApp:noEDFs', ...
                    'No .edf, .edf.gz, or .edf.zst files found in %s.', folder);
            end
        else
            warning('DYNAMOApp:badEDFPath', ...
    ...
    if ~isempty(opts.StagingPath)
        ...
        if isfolder(folder)
            ...
            if ~isempty(files)
                ...
                warning('DYNAMOApp:noCSVs', ...
                    'No .csv files found in %s.', folder);
            end
        else
            warning('DYNAMOApp:badStagingPath', ...
    ...
    if ~isempty(opts.Delimiter)
        ...
        switch d
            ...
            otherwise
                warning('DYNAMOApp:badDelimiter', ...
