...
%   output row — read_EDF needs the named-derivation prefix).
    ...
    function rows = extractSelectedRows(evt)
        % Preserves the order of evt.Selection so callers using
        % SelectionOrder='click' get click order, not sorted order.
        % `unique(..., 'stable')` keeps the first occurrence of each
        % row index in its original position.
        rows = [];
        try
            if isfield(evt, 'Selection') && ~isempty(evt.Selection)
                rows = unique(evt.Selection(:), 'stable');
