function refreshModeScatterDropdowns(app, axisKind)
    ...
    %     Power: X=SOpowerMean, Y=FreqMean, Size=Density, Color=PrefPhaseModel
    %     Phase: X=SOphaseMean, Y=FreqMean, Size=Density, Color=PrefPhaseModel
    ...
    switch axisKind
        case 'power'
            xPref  = 'SOpowerMean';
            cPrefs = {'PrefPhaseModel'};
        case 'phase'
            xPref  = 'SOphaseMean';
            % PrefPhaseModel isn't on phase paramfits (it's appended only
            % to power fits by annotatePowerWithPreferredPhase); fall
            % back to SOphaseMean when absent.
            cPrefs = {'PrefPhaseModel', 'SOphaseMean'};
        otherwise
            xPref  = 'Density';
            cPrefs = {'ID'};
    end
