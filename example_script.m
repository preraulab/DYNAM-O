...
%% Compute baseline spectrum used to normalize EEG data spectrum
artifacts = detect_artifacts(EEG, Fs); % detect artifacts in EEG
artifacts_stimes = logical(interp1(t, double(artifacts), stimes, 'nearest')); % get artifacts occuring at spectrogram timestamps
...
[SOpow_mat, freq_cbins, SOpow_cbins, ~, ~, peak_SOpow, peak_inds] = SOpower_histogram(EEG, Fs, peak_freqs, peak_times, 'stage_exclude', ...
                                                                        stage_exclude, 'artifacts', artifacts); 

% Compute SO phase
% use ('plot_flag', true) to plot directly from this function call
[SOphase_mat, ~, SOphase_cbins, ~, ~, peak_SOphase, ~] = SOphase_histogram(EEG, Fs, peak_freqs, peak_times, 'stage_exclude', ...
                                                            stage_exclude, 'artifacts', artifacts);
...
% Create empty figure 
figure; 
ax = figdesign(11,2,'type','usletter','orient','portrait','merge',{1:4, 5:10, 11:16, [17,19,21], [18,20,22]},...
               'margins',[0.03 .06 .055 .11 .13, 0.06]);
...
% Plot spectrogram
axes(ax(2))
imagesc(stimes, sfreqs, nanpow2db(spect)); % plot spectrogram
axis xy; % flip axes
colormap(ax(2), 'jet'); 
