...
% Validate the resolved time_range against the data and the staging.
% Cheap structural checks first, then the ones that actually prevent
% downstream NaN propagation into SOpowerHistogram / TFPeakHistogram.
assert(time_range(2) > time_range(1), ...
    'time_range = [%.3f %.3f] is empty or inverted; require time_range(2) > time_range(1).', ...
    time_range(1), time_range(2));
data_duration = (numel(data) - 1) / Fs;
assert(time_range(1) >= 0 && time_range(2) <= data_duration + 1/Fs, ...
    ['time_range = [%.3f %.3f] s extends outside the data [0 %.3f] s ' ...
     '(data has %d samples at Fs=%.3f Hz). time_range is in SECONDS; a ' ...
     'common mistake is passing [0 Fs*T] instead of [0 T].'], ...
    time_range(1), time_range(2), data_duration, numel(data), Fs);
assert(time_range(1) >= stage_times(1) && time_range(2) <= stage_times(end), ...
    ['time_range = [%.3f %.3f] s falls outside the staging window ' ...
     '[%.3f %.3f] s. The SO-power histogram step requires every ' ...
     'SOpower_times sample to fall inside stage_times; otherwise ' ...
     'interp1(..., ''previous'') returns NaN for the stage, and ' ...
     'TFPeakHistogram rejects the non-finite Cmetric_stages. Narrow ' ...
     'time_range to lie within stage_times, or extend stage_times to ' ...
     'cover the desired window.'], ...
    time_range(1), time_range(2), stage_times(1), stage_times(end));
