...
% Clamp the resolved time_range to the intersection of the data
% duration and the staging window. Without this, a time_range that
% runs past stage_times causes interp1(..., 'previous') in
% SOpowerHistogram to return NaN for trailing samples and
% TFPeakHistogram rejects the non-finite Cmetric_stages. Clamping is
% also forgiving about the seconds-vs-samples pitfall (callers who
% pass [0 Fs*T] by mistake end up with a full-recording analysis
% instead of a cryptic mid-pipeline error).
data_duration = (numel(data) - 1) / Fs;
range_lo = max([time_range(1), 0,              stage_times(1)]);
range_hi = min([time_range(2), data_duration,  stage_times(end)]);
if range_lo ~= time_range(1) || range_hi ~= time_range(2)
    warning('runDYNAMO:timeRangeClamped', ...
        ['time_range = [%.3f %.3f] s was clamped to [%.3f %.3f] s ' ...
         '(data [0 %.3f] s, stage_times [%.3f %.3f] s). If this was ' ...
         'unintended, check time_range units — it is in SECONDS, not ' ...
         'samples.'], ...
        time_range(1), time_range(2), range_lo, range_hi, ...
        data_duration, stage_times(1), stage_times(end));
    time_range = [range_lo, range_hi];
end
assert(time_range(2) > time_range(1), ...
    ['time_range collapsed to an empty interval after clamping ' ...
     'against data [0 %.3f] s and stage_times [%.3f %.3f] s — no ' ...
     'overlap with the recording to analyze.'], ...
    data_duration, stage_times(1), stage_times(end));
