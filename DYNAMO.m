            ...
            try
                ...
                if isempty(fitobj_phase)
                    ...
                    if ~isempty(obj.SOPHs.SOphase_paramfit.params)
                        ...
                        obj.SOPHs.SOphase_paramfit.peak_assign = ...
                            effective_peak_assign_label(opts_phase.peak_assign, 'phase');
                    end
                end
            catch ME_phase
                obj.SOPHs.SOphase_paramfit = [];
            ...
            try
                ...
                if isempty(fitobj_pow)
                    ...
                    if ~isempty(obj.SOPHs.SOpower_paramfit.params)
                        ...
                        obj.SOPHs.SOpower_paramfit.peak_assign = ...
                            effective_peak_assign_label(opts_pow.peak_assign, 'power');
                    end
                end
            catch ME_pow
                obj.SOPHs.SOpower_paramfit = [];
            ...
            function map = getDescMap(constructor)
                ...
                elseif strcmp(func2str(constructor), '@(x)param_basis_opts(''power'')')
                    map = struct(...
                        ...
                        'prefix_modes', 'Prefix modes fitting parameters [amp0, fmean0 (Hz), fstd0 (Hz), pmean0 (dB), pstd0 (dB), theta0 (rad)]; fstd0 and pstd0 are true standard deviations of the rotGauss kernel', ...
                        ...
                        'UB_default', 'Upper bounds [amp0, fmean0 (Hz), fstd0 (Hz), pmean0 (dB), pstd0 (dB), theta0 (rad)]; fstd0 and pstd0 are true standard deviations', ...
                        'LB_default', 'Lower bounds [amp0, fmean0 (Hz), fstd0 (Hz), pmean0 (dB), pstd0 (dB), theta0 (rad)]; fstd0 and pstd0 are true standard deviations', ...
                        'constrain_freq_center', 'Constrain frequency center (false: +/-Inf)', ...
                        'constrain_power_center', 'Constrain SO-power center (false: +/-Inf)', ...
                ...
                elseif strcmp(func2str(constructor), '@(x)param_basis_opts(''phase'')')
                    map = struct(...
                        ...
                        'wshed_exp', 'Watershed expansion flag', ...
                        'max_peaks', 'Maximum number of peaks to fit (-1 for unlimited)', ...
                        'prefix_modes', 'Prefix modes fitting parameters [amp0, fmean0 (Hz), fstd0 (Hz), pmean0 (rad), recikappa0 (rad), theta0 (rad)]; fstd0 is a true standard deviation, recikappa0 is 1/sqrt(kappa) and NOT a Gaussian standard deviation', ...
                        'prefix_modes_order', 'Prefix modes order (-1: after, 0: only, 1: before watershed)', ...
                        'max_overlap', 'Maximum allowed mode overlap', ...
                        'min_amp', 'Minimum amplitude for a peak', ...
                        'criterion', 'Model selection criterion (max, mindr2, minpctr2, kneedle)', ...
                        ...
                        'min_pctr2', 'Minimum percentage change in R-squared', ...
                        'kneedle_tol', 'Kneedle algorithm iteration tolerance', ...
                        'UB_default', 'Upper bounds [amp0, fmean0 (Hz), fstd0 (Hz), pmean0 (rad), recikappa0 (rad), theta0 (rad)]; fstd0 is a true standard deviation, recikappa0 is 1/sqrt(kappa)', ...
                        'LB_default', 'Lower bounds [amp0, fmean0 (Hz), fstd0 (Hz), pmean0 (rad), recikappa0 (rad), theta0 (rad)]; fstd0 is a true standard deviation, recikappa0 is 1/sqrt(kappa)', ...
                        'constrain_freq_center', 'Constrain frequency center (false: +/-Inf)', ...
                        'constrain_phase_center', 'Constrain SO-phase center (false: +/-Inf)', ...
                        'plot_on', 'Plot flag (0: none, 1: final, 2: iterations, 3: both)', ...
                        'SOPH_clim_prctiles', 'Heatmap color scaling percentiles [low, high]', ...
                        'verbose', 'Display detailed output');
                elseif strcmp(func2str(constructor), '@(x)spline_basis_opts(''power'')')
