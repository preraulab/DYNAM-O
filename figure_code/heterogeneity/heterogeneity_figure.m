%%% Plots spectrogram and rate histograms for 4 subjects to show heterogeneity among controls
...
results_folder=fullfile(base, '/preraugp/projects/spindle_detection/code/Lunesta histograms over time/results/Lunesta_poster_histograms_spectrograms');
...
segment_xlims = [[13832 13892];[17261 17321];[15010 15070];[5605 5665];...
    [22384 22444];[15346 15406];[23208 23268];[26901 26961];[14205 14265];... %[14216 14276]
...
segment_caxis = [[-20.1675 8.6379];[-15.9600 11.2222];[-13.5 8.9]%[-17.9102 7.0252];...
    ...
    [-18.6624 11.0005];[-18.6936 15.9467];[-19.0248 13.0286];[-17.5 13.5660];...
...
full_spectrogram_parameters.frequency_min = 0;
...
segment_spectrogram_parameters.frequency_min = 0;
...
% 
 for subject_number = [11] %[10,2,9,3] % old subjects: 10,2,11,3
    ...
    for night = 1:2
        ...
        [hist_mode_rates{subj_count}] = hist_mode_stats(hist_mode_lists{mode_list_idx}, sleep_rate_hist(night,:), bin_centers);
...
%     c.Label.VerticalAlignment = "bottom";    
    ...
    axes(ax(5));
    hold on;
    rate_hist_plots = plot(bin_centers, sleep_rate_hist);
    set(rate_hist_plots(spectrogram_night),'LineWidth',2);
    ...
    [pks,locs,pk_widths,pk_prominence]=findpeaks(sleep_rate_hist(2,:),bin_centers,'MinPeakHeight',0.0065,'MinPeakDistance',1.8,'Annotate','extents');
    ...
    vline(sub_locs,'linewidth', 1,'color', 'k', 'linestyle', '--');
    ...
    for i = 1 : size(hist_mode_list,1)
        inds = bin_centers>=hist_mode_list(i,1) & bin_centers<=hist_mode_list(i,2);
        
        mfreq = mean(bin_centers(inds));
        ...
        elseif mfreq>7
            ...
            color = [1 .9 .7];
                ...
                fillobj = fill([bin_centers(inds) fliplr(bin_centers(inds))],[sleep_rate_hist(2, inds) zeros(1,sum(inds))],color,...
            'EdgeColor','None');
    ...
    for ii = 1 : length(sub_locs)
        h = hline(sub_locs(ii),'linewidth',1,'color', 'k','linestyle', '--');
    end
    
    ax(5).Position = ax(4).Position;
    ax(5).Position(2) = .2;
    %print(night_comparison_figure,[subject_name,'-updated.eps'],'-depsc');
%     
