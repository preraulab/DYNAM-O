function power_dynamics_summary_recon(data, stages, stage_props, Fs, freq_range, SOpow, SOpow_times, SOpow_hist, SOpow_bins, SOpow_freqs, ...
                                peak_freqs, peak_times, peak_proms, time_window, fwindow_size, fsave_path, print_png, print_eps)
...
%       SOpow: 1D double - SO power time series 
%       SOpow_times: 1D double - SO power time stamps
...
%       time_window: 1x2 double -  [size, step] sliding window params for computing rate in reconstruction 
%       fwindow_size: double - frequency window size used for
%       reconstruction (Hz). Should be the same as frequency window used for SOpower histogram
...
assert(nargin >= 15, '15 inputs required')
...
[rate_true, rate_recon, rate_times, rate_SOP] = SOpow_hist_rate_recon(SOpow, SOpow_times, SOpow_hist, SOpow_bins, SOpow_freqs,...
...
% Generate axes
fh = figure;
ax = figdesign(7,2,'type','usletter','orient','portrait','merge',{1:2, 3:4, [5 7 9 ],[6 8 10], 11:12, 13:14},'margins', [.035 .025 .1 .08 .045 .05]);
...
axis xy;
th(5) = title('Observed TF-peak Density');
caxis(cx);
[~,sh] = scaleline(recon_axes(1), 3600,'1 Hour' );
sh.FontSize = 12;

ylabel('Frequency (Hz)')

letter_label(gcf,recon_axes(1),'E','l',lab_fsize,lab_gaps);

axes(recon_axes(2))
imagesc(rate_times,SOpow_freqs,rate_recon);
axis xy;
th(6) = title('Reconstructed TF-peak Density');
caxis(cx);
[~,sh] = scaleline(recon_axes(2), 3600,'1 Hour' );
sh.FontSize = 12;

ylabel('Frequency (Hz)')
%outerlabels([recon_axes(1), recon_axes(2)], '', 'Frequency (Hz)')

xlim([lightsout lightson]);


ylpow.Position(1) = ylscat.Position(1);
...
set(recon_axes,'fontsize', ax_fsize);
