...
%Slightly adjusted ROIs, decide on one for both figures
ROI_phase = [-pi/4 pi/2; (3*pi)/4 (3*pi)/2];
ROI_freq = [12 17; 6 11];

num_phase_ROIs = size(ROI_phase,1);

phase_bins_mod = [phase_bins, phase_bins+(2*pi)];
...
% progressbar
phists = cat(2, phasehists{elect_num},phasehists{elect_num});
...
parfor ii = 1:length(night_out)
    if night_inds(ii)
        ...
        if ~any(isnan(phase_hist(:)))
            mparams = fit_SOPphase_Gaussian_cos_model(phase_hist, phase_bins_mod, freqs_trunc, phase_regions, ROI_phase, ROI_freq, false);
%             title(num2str(ii));
            params(:,:,ii) = mparams;
            %pause
%             close all
...
disp(['Peaks rejected for being below amp threshold: ', num2str(sum(HC_amps<=min_amp)), ' out of ', num2str(length(HC_amps))]);
...
disp(['Peaks rejected for being below amp threshold: ', num2str(sum(SZ_amps<=min_amp)), ' out of ', num2str(length(SZ_amps))]);

%Wrap phases around
HC_phases(HC_phases>pi) = HC_phases(HC_phases>pi) - (2*pi);
SZ_phases(SZ_phases>pi) = SZ_phases(SZ_phases>pi) - (2*pi);
...
[pvals_issz, pvals_night] = mode_parameter_boxplot(params(:,:,night_inds), [1,2], [1,2,4], issz_out(night_inds), night_out(night_inds), {'Sigma Wide','Low'}, {'Amplitude','Frequency (Hz)', 'Phase (rad)'},...
                                                            0, 1, '', false, false);


mergefigures(figure(1), figure(2));
close([1,2])
