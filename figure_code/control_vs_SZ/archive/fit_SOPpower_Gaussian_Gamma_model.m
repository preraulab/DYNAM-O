function [params, offset, fitresult] = fit_SOPpower_Gaussian_Gamma_model(pow_hist, pow_bins, freq_bins, pow_ROIs, ROI_pows, ROI_freqs, plot_on)
...
var_names = {'amp','fmean','fstd','palpha','pbeta','v'};
...
for n = 1:N
    ...
    eqn_string = strcat(eqn_string, sprintf('%s * exp(-(((y - %s)/(2 * %s)).^2)) * (x^(%s-1) * exp(-%s * x)) + %s', peak_vars{:}));
    ...
    StartPoint(n,:) = [centroid_density,freq_centroid, 1,pow_centroid,1,0];
    Lower(n,:) = [0 ROI_freqs(n,1) 0 0 0 .01];
    Upper(n,:) = [max(ROI_data(:))*1.5 ROI_freqs(n,2) 10 10 10 max(ROI_data(:))] ;
...
for n = 1:N
    ...
    pbeta = params(n,5);
    V = params(n,6);
    Z = amp * exp(-(((Y - fmean)/(2 * fstd)).^2)) .* (X.^(palpha-1) .* exp(-pbeta .* X)) + V + offset;
    vol(n) = sum(Z(:));
end
