...
if col_norm
    ...
    for ii = 1:num_pow_ROIs
        %         clear TIB_curr
        %         TIB_curr(1,:,:) = TIBpow(:,pow_regions{ii,1})';
        %         region_rates = powhist_counts(pow_regions{ii,2}, pow_regions{ii,1},:) ./ TIB_curr;
        %         region_rates(isinf(region_rates)) = 0;
        %         region_rate_sum = squeeze(nansum(region_rates, [1,2]));
        %         pow_params(:,ii) = region_rate_sum;
