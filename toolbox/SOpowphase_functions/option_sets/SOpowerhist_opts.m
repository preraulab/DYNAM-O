...
addOptional(p, 'SOpower_min_time_in_bin', 0, @(x) validateattributes(x,{'numeric'},{'scalar','real','finite','nonnan','nonnegative','integer'}));
addOptional(p, 'SOpower_range', [-20,40], @(x) validateattributes(x,{'numeric', 'vector'}, {'real', 'nonempty'}));
addOptional(p, 'SOpower_binsizestep', [2,0.25], @(x) validateattributes(x,{'numeric', 'vector'}, {'real', 'nonempty'}));
