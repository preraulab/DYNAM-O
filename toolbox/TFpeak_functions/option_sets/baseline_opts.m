...
addOptional(p, 'baseline_stages',[1,2,3,4,5], @(x) validateattributes(x,{'numeric'},{'real','nonempty','vector'}));
%Logical indices for time points to exclude for baseline computation
addOptional(p, 'baseline_exclude',logical([]), @(x) validateattributes(x,{'logical'},{'real','finite','nonnan','2d'}));
%Percentile to use for fixed baseline computation
addOptional(p, 'baseline_ptile',2, @(x) validateattributes(x,{'numeric'},{'real','nonempty','scalar'}));
