...
addOptional(p, 'baseline_stages',[1,2,3,4,5],@(x) validateattributes(x,{'vector','numeric'},{'real','nonempty'}));
%Logical indices for time points to exclude for baseline computation
addOptional(p, 'baseline_exclude',logical([]),@(x) validateattributes(x,{'vector','logical'},{'real','finite','nonnan'}));
%Percentile to use for fixed baseline computation
addOptional(p, 'baseline_ptile',2,@(x) validateattributes(x,{'scalar','numeric'},{'real','nonempty'}));
...
addOptional(p, 'baseline_trim',[-inf inf],@(x) validateattributes(x,{'vector','numeric'},{'real'}));
