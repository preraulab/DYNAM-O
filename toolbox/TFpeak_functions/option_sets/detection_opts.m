...
addOptional(p, 'dsfreqs', 0.1, @(x) validateattributes(x,{'numeric'},{'real','nonempty','nonnan','scalar'}));
%Taper parameters [time half-bandwidth product, number of tapers]
addOptional(p, 'mtm_taper_params', [2, 3], @(x) validateattributes(x,{'numeric'},{'real','nonempty','nonnan','vector','numel',2}));
%Window length in spectrogram computation for the first round of watershed
addOptional(p, 'mtm_window_length_1', 1, @(x) validateattributes(x,{'numeric'},{'real','nonempty','nonnan','scalar'}));
%Window length in spectrogram computation for the second round of watershed
addOptional(p, 'mtm_window_length_2', 2, @(x) validateattributes(x,{'numeric'},{'real','nonempty','nonnan','scalar'}));
%Window step size in spectrogram computations
addOptional(p, 'mtm_window_stepsize', 0.05, @(x) validateattributes(x,{'numeric'},{'real','nonempty','nonnan','scalar'}));
...
addOptional(p, 'quality_setting', 'default', @(x) validateattributes(x,{'char'},{'nonempty'}));

%Merging and trimming parameters
%Maximum number of merges to perform
addOptional(p, 'max_merges', inf, @(x) validateattributes(x,{'numeric'},{'real','nonempty','nonnan','scalar'}));
%Fraction maximum trimmed volume
addOptional(p, 'trim_vol', 0.8, @(x) validateattributes(x,{'numeric'},{'real','nonempty','nonnan','scalar'}));
%Max duration allowed
addOptional(p, 'dur_max', 5, @(x) validateattributes(x,{'numeric'},{'real','nonempty','nonnan','scalar'}));
%Max bandwidth allowed
addOptional(p, 'bw_max', 15, @(x) validateattributes(x,{'numeric'},{'real','nonempty','nonnan','scalar'}));
