function stats_table = computePeakStatsTable(regions,boundaries,data,xvalues,yvalues,segment_num, properties)
...
%   properties -- cell array of properties to include, can be any subset of
...
if nargin<7 || isempty(properties) || any(strcmpi(properties,'all'))
    properties = {'Area', 'Bandwidth', 'Boundaries', 'BoundingBox', 'Duration', 'Height', 'HeightData', 'PeakFrequency', 'PeakTime', 'SegmentNum', 'Volume'};
end
...
r_props = {};
if any(strcmpi(properties,'Area'))
    r_props = cat(2,r_props,'Area');
end

if any(strcmpi(properties,'BoundingBox')) || any(strcmpi(properties,'Bandwidth')) || any(strcmpi(properties,'Duration'))
    r_props = cat(2,r_props,'BoundingBox');
end

if any(strcmpi(properties,'PeakTime')) || any(strcmpi(properties,'PeakFrequency')) 
    r_props = cat(2,r_props,'WeightedCentroid');
end

if any(strcmpi(properties,'Height')) || any(strcmpi(properties,'HeightData')) || any(strcmpi(properties,'Volume')) 
...
%% Compute statistics
%Bounding Box
if any(strcmpi(properties,'BoundingBox')) || any(strcmpi(properties,'Duration')) || any(strcmpi(properties,'Bandwidth'))
...
%Volume
if any(strcmpi(properties,'Volume'))
...
%Boundaries
if any(strcmpi(properties,'Boundaries'))
...
%Peak Time
if any(strcmpi(properties,'PeakTime'))
...
%Peak Frequency
if any(strcmpi(properties,'PeakFrequency'))
...
%Height
if any(strcmpi(properties,'Height'))
...
%Duration
if any(strcmpi(properties,'Duration'))
...
%Bandwidth
if any(strcmpi(properties,'Bandwidth'))
...
%Segment Number
if any(strcmpi(properties,'SegmentNum'))
...
%Region data
if any(strcmpi(properties,'HeightData'))
...
if ~any(strcmpi(properties,'BoundingBox'))
