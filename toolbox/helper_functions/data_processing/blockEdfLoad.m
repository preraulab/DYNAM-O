...
if nargout >= 2
    try
        ...
        A = fread(fid, edfSignalHeaderSize);
    catch exception
        msg = 'File load error. Check available memory.';
        error(msg);
    end
...
if nargout >=3
    ...
    try
        ...
        if nargin == 1
            ...
            edfSignalLabels = {signalHeader.signal_labels};
            signal_inds = cellfun(@(x) find(strcmpi(edfSignalLabels, x)), signalLabels, 'UniformOutput', false);

            empty_inds = cellfun(@isempty,signal_inds);

            %Throw a warning if there are invalid labels
            if any(empty_inds)
                warning(['Invalid signal labels: ' sprintf('%s ',string(signalLabels(empty_inds))) newline '         Valid labels: ' sprintf('%s ',string(edfSignalLabels))]);
            end

            %Throw a warning and return if no valid labels
            if all(empty_inds)
                warning(['Invalid signal labels: ' sprintf('%s ',string(signalLabels(empty_inds)))])
            ...
            signalIndexes = cell2mat(signal_inds(~empty_inds));
            ...
            %Vectorized appending of locations
            loc_inds = arrayfun(@(x,y)x:y,startLocs(signalIndexes),endLocs(signalIndexes),'UniformOutput',false);
            signalLocs = cat(2,loc_inds{:});
            ...
            for r = 1:header.num_data_records
                a = fread(fid, edfRecordSize, 'int16');
